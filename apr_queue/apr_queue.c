...
struct apr_queue_t
{
    void **data;         // 二级指针，实现一个线性队列
    unsigned int nelts;  /**< # elements */
    unsigned int in;     /**< next empty location 指向入队元素*/
    unsigned int out;    /**< next filled location 指向出队元素*/
    unsigned int bounds; /**< max size of queue */
    unsigned int full_waiters;
    unsigned int empty_waiters;
    unsigned int item_size; // 表示队列之中元素的大小，用字节数来衡量
...
apr_queue_create(apr_queue_t **q,
                 ...
                 apr_pool_t *a)
{
    ...
    *q = queue;
    ...
    queue->data = apr_pcalloc(a, queue_capacity * sizeof(void *)); // 创建的是一个指针数组，指针的大小一般是一样的
...
apr_queue_push(apr_queue_t *queue, void *data)
{
    ...
    queue->data[queue->in] = data; // 二级指针指向的数组，存放指向的内存的一级指针
    queue->in++;
 ...
 */
APU_DECLARE(apr_status_t)
apr_queue_trypush(apr_queue_t *queue, void *data)
{
    apr_status_t rv;
 ...
 * not thread safe
 */
APU_DECLARE(unsigned int)
apr_queue_size(apr_queue_t *queue)
{
    return queue->nelts;
}
/**
 ...
 */
APU_DECLARE(unsigned int)
apr_queue_item_size(apr_queue_t *queue)
{
 ...
 */
APU_DECLARE(apr_status_t)
apr_queue_trypop(apr_queue_t *queue, void **data)
{
    apr_status_t rv;
...
APU_DECLARE(apr_status_t)
apr_queue_interrupt_all(apr_queue_t *queue)
{
    apr_status_t rv;
...
APU_DECLARE(apr_status_t)
apr_queue_term(apr_queue_t *queue)
{
    apr_status_t rv;
